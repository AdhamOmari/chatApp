...
export default function ConversationCard({ user }: ConversationCardProps) {
    ...
    return (
        <div
            onClick={handleClick}
            className={`w-11/12 relative h-20 ${theme === 'on' ? 'bg-white hover:bg-gray-300 text-black' : 'bg-slate-800 hover:bg-slate-950 text-white'} my-2 flex items-center cursor-pointer rounded-2xl px-4 mx-4 justify-between transition-all duration-700`}
        >
            ...
            <div className="flex flex-col items-start justify-center flex-grow ml-2">
                <h1 className="font-semibold">{user.name}</h1>
                <p className="text-sm font-medium">ProgrammerPath : {user.ProgrammerPath}</p>
            </div>
            {typing && (
                <p className={`text-xs absolute bottom-2 left-20 ${theme === 'on' ? 'text-black' : 'text-white'} tracking-widest font-semibold`}>
                    Typing...
                </p>
            )}
        </div>
